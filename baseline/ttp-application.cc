    ...
    */
    ...
    TTPApplication::DirectTrustCalculator(Address from, Address to)
    {
        ...
        for(auto window : direct_trust_repo[from])
        {
            ...
            {
                p += (exp(-(0.7)*time_diff) * p + window[to].first);
                n += (exp(-(0.7)*time_diff) * n + window[to].second);
    ...
    void TTPApplication::ComputeTrust(Address from, Address to)
    {
        ...
        if(recommenders.size() > 1)
        {
            ...
            if(avg_dt_rk >= 0.5)
            {
                ...
                // time_diff = (Simulator::Now().GetSeconds() - last_int_time.GetSeconds())/20.0;
                time_diff = (Simulator::Now() - last_int_time).GetMinutes();
                // cout<<time_diff<<"\t"<<interaction<<endl;
                double omega = 1 - pow(0.1, exp(-(time_diff) * interaction));
    ...
    //    TODO::
        ...
        cout<<InetSocketAddress::ConvertFrom(from).GetIpv4()<<"\t"<<Now().GetSeconds()<<"\t"<<trust_score<<"\t"<<1-trust_score<<endl;
