...
main (int argc, char *argv[])
{
  ...
  int nGoodNode = 105;
  // 135 good node and 5 sp node
  
  // 0->91 these 92 iot device are good
  // 92->99 these 8 are good sp

  int nBadNode = 45;
  // for 10% bad node should be 15
  // for 20% bad node should be 30
  // for 40% bad node should be 60
  // for 50% bad node should be 75
  // for 60% bad node should be 90
  ...
  // {
  //   spApps[i] = CreateObject<SPApplication> ();
  //   spApps[i]->SetStartTime (Seconds (1.0));
  //   spApps[i]->SetStopTime (Seconds (simTime));
  //   spApps[i]->Setup (good_sp_speaker[i],good_sp_listener[i] , 1040, 1000, DataRate ("1Mbps"),sinkPort);

  //   for(int j = 0; j < nGoodNode; j++){
  //       spApps[i]->AddIoTAddr(goodIoTAddr[j]);
  //     }
  //   for(int j = 0; j < nBadNode; j++){
  //       spApps[i]->AddIoTAddr(badIoTAddr[j]);
  //     }
    
  //   goodSPContainer.Get (i)-> AddApplication(spApps[i]);
  // }
